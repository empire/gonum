...
func (u *urna) relabel(dst, src []*Statement) ([]*Statement, error) {
	...
	sort.Strings(lexicalHashes)
	for _, hash := range lexicalHashes {
		...
		for _, b := range termsFor[hash] {  // 6.2
			...
			// 6.2.4
			hash, ni := u.hashNDegreeQuads(b, names)
			paths[string(hash)] = append(paths[string(hash)], ni)
		}
		...
		for _, hash := range lexicalHashPaths {
	...
	permutations:
		for p.next() {
			...
			for _, b := range work { // 5.4.5
				hash, cand := u.hashNDegreeQuads(b, namesCopy) // 5.4.5.1
...
// hashToRelatedURDNA2015 is the section 1., 2. and 3. of 4.8.2 of the spec
// at https://json-ld.github.io/rdf-dataset-canonicalization/spec/index.html#hash-n-degree-quads
// for URGNA2012 https://json-ld.github.io/rdf-dataset-canonicalization/spec/index.html#urgna2012.
func (u *urna) hashToRelatedURGNA2012(b string, names *issuer) map[string][]string {
	...
	for _, s := range u.statementsFor[b] { // 2. and  3.
		...
		switch {
		case strings.HasPrefix(s.Subject.Value, "_:") && s.Subject.Value != b: // 3.1
			term = s.Subject.Value
			pos = 'p'
		case strings.HasPrefix(s.Object.Value, "_:") && s.Object.Value != b: // 3.2
			term = s.Object.Value
			pos = 'r'
		default:
			continue
